<template>
  <div class="main">
    <div class="left">
      ...
      <article class="list">
        ...
        <span class="custom-tree-node" @click="selectedGroup(data)">
          <span style="margin-right: 10px">{{ data.groupName }}</span>
          <span style="margin-right: auto">({{ data.questionNumber }})</span>
          <el-icon title="课程转移" v-if="data.parent !== '2' && checkHasRole(roleKeys.transGroup)" @click="transGroup(data)"><i-switch/></el-icon>
        </span>
          </template>
        </el-tree>
      </article>
    ...
    <div class="right">
      <div class="search">
        ...
        <span class="group">当前课程:{{ data.selectedGroup.groupName }}</span>
      </div>
      <el-table v-if=data.selectedGroup :data="data.tableData" border style="width: 100%" v-loading="data.tableLoad">
          ...
          <template #default="scope">
            <div class="table-operate">
              ...
              <el-divider direction="vertical"
                          v-if="checkHasRole(roleKeys.edit) && checkHasRole(roleKeys.del)"></el-divider>
      ...
      <Empty v-if="!data.selectedGroup"></Empty>
    </div>
  </div>
  <t-dialog
      v-model:visible="data.groupVisible" :header="data.editGroupType === 'add' ? '添加课程':'编辑课程'"
    ...
    <el-form :model="data.groupParam" ref="groupForm" style="margin-top: 20px">
      <el-form-item label="课程名" prop="groupName" :rules="[{required: true,message: '课程名不能为空'}]">
        <el-input v-model="data.groupParam.groupName" placeholder="请输入课程名"></el-input>
      </el-form-item>
    </el-form>
  </t-dialog>
  <t-dialog
      v-model:visible="data.groupTransVisible" :header="data.editGroupType === 'add' ? '添加课程':'编辑课程'"
      width="400px" v-if="data.groupTransVisible" :on-confirm="onConfirmTransDia" :on-close="closeTransDia"
      :confirm-btn="{content: '保存',theme: 'primary',loading: data.diaLoad,}">
    <el-form :model="data.groupParam" ref="groupForm" style="margin-top: 20px">
      <el-form-item label="课程名" prop="groupName" :rules="[{required: true,message: '课程名不能为空'}]">
        <el-input v-model="data.groupParam.groupName" placeholder="请输入课程名" clearable></el-input>
      </el-form-item>
    </el-form>
  </t-dialog>
  <!--  课程转移-->
  <el-dialog v-model="data.transGroupVisible" title="转移课程" width="400px" :before-close="closeTransDia">
    <div style="padding-left: 20px">{{ `当前所属课程:  ${data.transFromGroup.groupName}` }}</div>
    <el-form :model="data.transGroup" ref="groupTransForm" style="margin-top: 20px">
      <el-form-item label="转移至课程" prop="id" :rules="[{required: true,message: '请选择课程'}]">
        <!--        <el-select style="width: 100%" v-model="data.transGroup.id" placeholder="请选择课程" filterable>
        ...
        <el-cascader
            style="width: 100%"
            placeholder="请选择课程"
            v-model="data.transGroup.id"
            :options="data.groupList.map(i => {
  ...
  <!--  试题分类更改-->
  <el-dialog v-model="data.changeGroupVisible" title="更改课程" width="400px" :before-close="closeChangeGroupDia">
    <div style="padding-left: 20px">{{ `当前所属课程:  ${data.selectedGroup.groupName}` }}</div>
  ...
  <t-drawer v-model:visible="data.editShow" :header="data.editType === 'add' ? '添加试题':'编辑试题'" size="700px"
            ...
            :confirm-btn="{content: '保存',theme: 'primary',loading: data.diaLoad,}">
    <edit-question v-if=data.editShow ref="editQuestion" :edit-type="data.editType" :id="data.editId"
                   :group="data.addQuestionGroup"></edit-question>
  </t-drawer>
</template>
...
const delGroup = item => {
  if (item.questionNumber > 0) {
    ElMessage.warning('改课程下还有试题，请转移分组后再删除')
    return;
  }
...
const addQuestion = () => {
  ...
  if (!data.selectedGroup.id || data.selectedGroup.parent === '2') {
    ElMessage.warning('请选择课程小类')
    return
  }
  ...
  groupForm.value.validate(async (checkRes) => {
    if (checkRes) {
      ...
      if (data.editGroupType === 'add') {
        res = await get(`/question/group/add/${data.groupParam.groupName}/${data.parent}`)
        message = '添加成功';
      } else {
...
const confirmTransDia = () => {
  groupTransForm.value.validate(async (checkRes) => {
    if (checkRes) {
      if (data.transFromGroup.id === data.transGroup.id[1]) {
        closeTransDia();
        return;
      }
      data.diaLoad = true;
      let res = await get(`/question/group/trans?from=${data.transFromGroup.id}&to=${data.transGroup.id[1]}`)
      data.diaLoad = true;
      if (res.status === 1000) {
...
const confirmChangeGroupDia = () => {
  changeGroupForm.value.validate(async (checkRes) => {
    if (checkRes) {
      ...
      data.diaLoad = true;
      let res = await post(`/question/edit`, {id: data.changeGroupId, groupId: data.changeGroup.id[1]})
      data.diaLoad = false;
      if (res.status === 1000) {
        ElMessage.success('更改课程成功');
        closeChangeGroupDia();
        await getQuestionList();
        await getGroupList();
      } else {
...
const onConfirmEditDia = async () => {
  await editQuestion.value.save();
  closeEditDia();
}
